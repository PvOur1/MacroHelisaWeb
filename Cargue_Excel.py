...
def apply_resolution(name: str):
    ...
    """
    global REGION_ACEPTAR, REGION_BTN1, REGION_BTN2, REGION_BTN4, REGION_BTN5, REGION_TERCERO
...
def process_all_rows():
    ...
    for idx, row in CURRENT_DF.iterrows():
        ...
        # 5) Post-proceso por fila (ajusta a tu flujo)
        time.sleep(2)
...
def process_rows_from(start_idx):
    ...
    for idx, row in df_from.iterrows():
        if STOP_EXCEL.is_set():
            set_status("Procesamiento detenido por usuario")
            break
...
def process_selected_rows_clicked():
    """Procesa solo las filas seleccionadas en la tabla."""
    sel = tree.selection()
    if not sel:
        messagebox.showinfo("Seleccionar filas", "Selecciona al menos una fila en la tabla.")
        return
    ...
    if not selected_indices:
        messagebox.showwarning("Error", "No se pudo determinar los índices de las filas seleccionadas.")
        return
...
btn_selected = tk.Button(top_frame2, text="▶ Procesar seleccionadas", width=22, command=process_selected_rows_clicked)
...
def edit_regions_dialog(sel=None):
    ...
    entries = {}
    keys = ["REGION_ACEPTAR", "REGION_BTN1", "REGION_BTN2", "REGION_BTN4", "REGION_BTN5", "REGION_TERCERO"]
